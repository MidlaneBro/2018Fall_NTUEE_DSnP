...
CmdParser::parseCmd(string& option)
{
   ...
   if(e==0){
      ...
      if (n != string::npos) {
          option=str.substr(n+1,str.size()-n-1);
      }
      else{
        option="";
      }
...
CmdParser::listCmd(const string& str)
{
  ...
  if(begin==string::npos){//Null string
     int t=0;
     cout<<endl;
     for(CmdMap::const_iterator iter=_cmdMap.begin();iter!=_cmdMap.end();++iter){
  ...
  string s=str.substr(begin,str.size()-begin);
  string tok;
  size_t space_position=myStrGetTok(s,tok);
  ...
  if(space_position==string::npos){//No space before the cursor
    if(match.empty()){//no match command
      mybeep();
      return;
    }
    else if(match.size()==1){//only one possibility
      string cmd=(match[0]->first)+(match[0]->second->getOptCmd());
      cmd=cmd.substr(tok.size(),cmd.size()-tok.size());
      for(int i=0;i<cmd.size();i++){
        insertChar(cmd[i]);
      }
      insertChar(' ');
  ...
  else{//at least one space before the cursor
    if(getCmd(tok)!=0){//first word match single command
      assert(_tabPressCount!=0);
      if(_tabPressCount==1){//press tab for first time
        cout<<endl;
        match[0]->second->usage(cout);
        reprintCmd();
        return;
      }
      else{//press tab for more than one time
        if(s.back()==' '){//the character before the cursor is a space
          vector<string> files;
          listDir(files,"",".");
          if(files.size()==1){//only one file in directory
          ...
          int i;
          for(i=1;i<=files[0].size();i++){
            bool flag=false;
            for(int j=1;j<files.size();j++){
              if(files[j].find(files[0].substr(0,i))!=0){
                flag=true;
                break;
              }
            }
          ...
          else{//there is a common prefix length i-1
              string prefix=files[0].substr(0,i-1);
        ...
        else{//the character before the cursor is not a space
          string prefix;
          size_t last_space=s.find_last_of(" ");
          prefix=s.substr(last_space+1,s.size()-last_space-1);
          vector<string> files;
          listDir(files,prefix,".");
          if(files.empty()){//no matched file for the prefix
            mybeep();
            return;
          }
          if(files.size()==1){//only one file in directory
            for(int i=prefix.size();i<files[0].size();i++){
              insertChar(files[0][i]);
            }
            insertChar(' ');
            return;
          ...
          int i;
          for(i=prefix.size()+1;i<=files[0].size();i++){
            bool flag=false;
            for(int j=1;j<files.size();j++){
              if(files[j].find(files[0].substr(0,i))!=0){
                flag=true;
          ...
          if(i==prefix.size()+1){//no more common prefix
              cout<<endl;
              for(int j=0;j<files.size();j++){
                if(j%5==0&&j!=0){
                  cout<<endl;
          ...
          else{//there are more common prefix
              string extra_prefix=files[0].substr(prefix.size(),i-prefix.size()-1);
              for(int j=0;j<extra_prefix.size();j++){
                insertChar(extra_prefix[j]);
              }
              mybeep();
              return;
          }
...
CmdParser::getCmd(string cmd)
{
   ...
   for(CmdMap::iterator iter=_cmdMap.begin();iter!=_cmdMap.end();++iter){
		if(cmd.find(iter->first)==0){
			string complete_command = iter->first + iter->second->getOptCmd();
			for(int i=0;i<complete_command.size();i++){
				complete_command[i]=toupper(complete_command[i]);
			}
			if(complete_command.find(cmd)==0){
				e = iter->second;
			}
		}
	}
