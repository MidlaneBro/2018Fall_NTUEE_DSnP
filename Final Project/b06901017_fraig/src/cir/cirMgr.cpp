...
/**************************************************************/
/*      class CirMgr member functions for circuit access      */
/**************************************************************/
...
CirMgr::readCircuit(const string& fileName)
{
   ...
   s.clear();
   for(unsigned i=4;i<sizeof(buf);i++){
      if(buf[i]==0||isspace(buf[i])){
         break;
      }
      s+=buf[i];
   ...
   for(int i=1;i<5;i++){
      ...
      s.clear();
      for(unsigned j=colNo;j<sizeof(buf);j++){
   ...
   for(int i=0;i<miloa[4];i++){
      ...
      for(int t=1;t<3;t++){
         ...
         s.clear();
         for(unsigned i=colNo;i<sizeof(buf);i++){
            if(buf[i]==0||isspace(buf[i])){
               break;
            }
            s+=buf[i];
   ...
   for(unsigned i=0;i<net_record[0].size();i++){
   ...
   for(unsigned i=0;i<po_record.size();i++){
...
CirMgr::printFloatGates() const
{
   ...
   if(!type1.empty()){
      ...
      for(unsigned i=0;i<type1.size();i++){
         cout<<" "<<type1[i];
      }
      cout<<endl;
   }
   if(!type2.empty()){
      ...
      for(unsigned i=0;i<type2.size();i++){
         cout<<" "<<type2[i];
      }
      cout<<endl;
   }
...
CirMgr::printFECPairs() const 
{
   ...
   for(unsigned i=0;i<fecGrps->size();i++){
      ...
      for(unsigned j=0;j<(*fecGrps)[i]->size();j++){
         cout<<" ";
         if((*(*fecGrps)[i])[j]->getsignal()!=(*(*fecGrps)[i])[0]->getsignal()){
            cout<<"!";
         }
         cout<<(*(*fecGrps)[i])[j]->getVarId();
      }
      cout<<endl;
   }
}
...
/**********************************************************/
/*  class CirMgr member functions for programming helping */
/**********************************************************/
...
//delete this gate
//maintain idlist and gatelist
//this gate will be erased from its fanin-gate's fanout list
//this gate on its fanout-gate's fanin list will become zero
void
CirMgr::remove(CirGate* g){
  assert(g->getTypeStr()=="AIG"||g->getTypeStr()=="UNDEF");
  if(g->getTypeStr()=="AIG"){
    for(int i=miloa[1]+miloa[3];i<miloa[1]+miloa[3]+miloa[4];i++){
      if(idlist[i]==g->getVarId()){
  ...
  if(g->getTypeStr()=="UNDEF"){
    for(unsigned i=miloa[1]+miloa[3]+miloa[4]+1;i<idlist.size();i++){
      if(idlist[i]==g->getVarId()){
        idlist.erase(idlist.begin()+i);
        break;
      }
  ...
  for(unsigned i=0;i<fanin.size();i++){
    fanin[i]->gate()->removeFanout(g);
  }
  for(unsigned i=0;i<fanout.size();i++){
    fanout[i]->gate()->removeFanin(g);
  }
  gatelist[g->getVarId()]=0;
  delete g;
}

void
CirMgr::print_idlist() const
{
   for(unsigned i=0;i<idlist.size();i++){
    cout<<idlist[i]<<" ";
   }
...
void
CirMgr::print_gatelist() const
{
   for(unsigned i=0;i<gatelist.size();i++){
      if(gatelist[i]){
         cout<<gatelist[i]->getVarId()<<" ";
      }
      else{
         cout<<gatelist[i]<<" ";
      }
...
bool
CirMgr::mysort1(CirGate* a,CirGate* b)
{
   return a->getVarId() < b->getVarId();
}

bool
CirMgr::mysort2(GateList* a,GateList* b)
{
   return (*a)[0]->getVarId() < (*b)[0]->getVarId();
}